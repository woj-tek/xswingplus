...
public class AIInterface implements Updateable {
	...
	private LinkedList<Integer> zuege = new LinkedList<Integer>();
	private Ball lastBall = null;
	private XSwingListener listener;
	...
	private void bestePositionBerechnen() {
		...
		// System.out.println("Debug - " + baelle + " Bälle:\n" + ballTable.printBallTable());
		zuege.add(0);
		zuege.add(0);
		zuege.add(0);
		for (int i = 0; i < 8; i++) {
			if (ballTable.getColumnHeight(i) == 8) {
				continue; // Game over, nicht berücksichtigen
			}
